...
try:
    ...
    # Support custom model paths from extra_model_paths.yaml
    try:
        hy_motion_paths = folder_paths.get_folder_paths("hy_motion")
    except KeyError:
        hy_motion_paths = []
    if hy_motion_paths:
        preferred = None
        fallback = None
        for path in hy_motion_paths:
            ckpts_dir = (
                path
                if os.path.basename(path).lower() == "ckpts"
                else os.path.join(path, "ckpts")
            )
            ...
            if fallback is None:
                fallback = path
            if os.path.exists(
                os.path.join(ckpts_dir, "tencent", "HY-Motion-1.0", "config.yml")
            ) or os.path.exists(
                os.path.join(ckpts_dir, "tencent", "HY-Motion-1.0-Lite", "config.yml")
            ):
                preferred = path
                break
        HYMOTION_MODELS_DIR = preferred or fallback or hy_motion_paths[0]
    else:
...
def _hymotion_complete(model_dir: str) -> bool:
    """True only if config.yml exists and latest.ckpt looks non-truncated.

    An interrupted download can leave a partial latest.ckpt that passes a
    plain existence check but fails at load time; treat anything under 1 MiB
    as broken (real checkpoints are hundreds of MB).
    """
    config_path = os.path.join(model_dir, "config.yml")
    ckpt_path = os.path.join(model_dir, "latest.ckpt")
    if not (os.path.isfile(config_path) and os.path.isfile(ckpt_path)):
        return False
    try:
...
class HYMotionLLMWrapper:
    ...
    def __init__(
        self, model, tokenizer, llm_type="qwen3", max_length=512, crop_start=0
    ):
        self.model = model
        ...
        self.hidden_size = (
            model.config.hidden_size if hasattr(model, "config") else 4096
        )
...
class HYMotionData:
    ...
    def __init__(
        self, output_dict: Dict[str, Any], text: str, duration: float, seeds: List[int]
    ):
        ...
        self.seeds = seeds
        self.batch_size = (
            output_dict["keypoints3d"].shape[0] if "keypoints3d" in output_dict else 1
        )
...
class HYMotionLoadLLM:
    ...
    @classmethod
    def INPUT_TYPES(s):
        # Always include common models, then add any discovered folders
        qwen_models = [
            "Qwen3-8B",
            "Qwen3-8B-bnb-4bit",
            "Qwen3-8B-AWQ",
        ...
        bnb_model_path = os.path.join(_hy_ckpts_dir(), "Qwen3-8B-bnb-4bit")
        ...
        awq_model_path = os.path.join(_hy_ckpts_dir(), "Qwen3-8B-AWQ")
        if os.path.exists(awq_model_path):
            if "Qwen3-8B-AWQ" not in qwen_models:
                qwen_models.append("Qwen3-8B-AWQ")

        # Default to Qwen3-8B if no models found (safety)
        if not qwen_models:
            qwen_models = ["Qwen3-8B"]

        return {
            "required": {
                "model_name": (qwen_models, {"default": "Qwen3-8B-bnb-4bit"}),
                "quantization": (
                    ["none", "int8", "int4", "bnb-4bit", "awq"],
                    {"default": "none"},
                ),
    ...
    RETURN_NAMES = ("llm",)
    FUNCTION = "load_llm"
    CATEGORY = "OpenBlender/HY-Motion/Loaders"

    @staticmethod
    def _candidate_repos_for_model(model_name: str):
        ...
        # Common quantized naming variants
        if "bnb-4bit" in base.lower():
            candidates.insert(0, f"unsloth/{base}")
        if "awq" in base.lower():
            candidates.insert(0, f"Qwen/{base}")
    ...
    @staticmethod
    def _model_dir_complete(local_path: str) -> bool:
        """True only if the LLM dir has the files needed to load AND the
        config looks like a Qwen model (guards against interrupted downloads
        or wrong-repo snapshots, which previously produced cryptic errors
        like KeyError: BitConfig deep inside transformers)."""
        if not os.path.isdir(local_path):
            return False
        config_file = os.path.join(local_path, "config.json")
        tokenizer_file = os.path.join(local_path, "tokenizer_config.json")
        if not (os.path.isfile(config_file) and os.path.isfile(tokenizer_file)):
            return False
        try:
            import json
            ...
            if "qwen" not in model_type:
                print(
                    f"[HY-Motion] Wrong model content in {local_path} "
                    f"(model_type={model_type!r}, expected a Qwen model)"
                )
                return False
        except Exception:
            return False
    ...
    @classmethod
    def _ensure_local_model_dir(cls, model_name: str, local_path: str):
        if cls._model_dir_complete(local_path):
            return
        if os.path.isdir(local_path):
            # Incomplete or corrupt download (e.g. interrupted snapshot leaves
            # an empty/partial dir that would silently skip re-download)
            print(f"[HY-Motion] Removing incomplete model dir: {local_path}")
            import shutil
        ...
        for repo_id in cls._candidate_repos_for_model(model_name):
            try:
                ...
                if cls._model_dir_complete(local_path):
                    ...
                    return
                print(f"[HY-Motion] Download from {repo_id} looks incomplete, trying next repo...")
            except Exception as e:
                last_err = e
        if last_err is not None:
            print(f"[HY-Motion] Auto-download failed for '{model_name}': {last_err}")

    def load_llm(
        self, model_name="Qwen3-8B", quantization="none", offload_to_cpu=False
    ):
        from transformers import AutoModelForCausalLM, AutoTokenizer, BitsAndBytesConfig
        from .hymotion.network.text_encoders.model_constants import (
            PROMPT_TEMPLATE_ENCODE_HUMAN_MOTION,
        )

        local_path = os.path.join(_hy_ckpts_dir(), model_name)
        self._ensure_local_model_dir(model_name, local_path)
        if not self._model_dir_complete(local_path):
            raise FileNotFoundError(
                f"LLM directory missing or incomplete: {local_path}. "
                f"Delete the folder and re-run to re-download {model_name}, "
                f"or place the model files there manually (config.json, "
                f"tokenizer_config.json, weights)."
            )

        print(
            f"[HY-Motion] Loading LLM: {local_path}, quantization={quantization}, offload_to_cpu={offload_to_cpu}"
        )
        ...
        if offload_to_cpu:
            ...
            try:
                ...
                tokenizer = AutoTokenizer.from_pretrained(
                    local_path, padding_side="right", local_files_only=True
                )
                ...
                template = [
                    {
                        "role": "system",
                        "content": f"{PROMPT_TEMPLATE_ENCODE_HUMAN_MOTION}",
                    },
                    {"role": "user", "content": "{}"},
                ]
        ...
        wrapper = HYMotionLLMWrapper(
            model=model, tokenizer=tokenizer, max_length=512, crop_start=crop_start
        )
        print(f"[HY-Motion] LLM loaded, hidden_size={wrapper.hidden_size}")
        return (wrapper,)
    ...
    def _compute_crop_start(self, tokenizer, template) -> int:
        ...
        marker = "<BOC>"
        msgs = [
            {"role": "system", "content": template[0]["content"]},
            {"role": "user", "content": marker},
        ]
        s = tokenizer.apply_chat_template(
            msgs, tokenize=False, add_generation_prompt=False, enable_thinking=False
        )
        full_ids = tokenizer(s, return_tensors="pt", add_special_tokens=True)[
            "input_ids"
        ][0].tolist()
...
class HYMotionLoadLLMGGUF:
    ...
    def INPUT_TYPES(s):
        ...
        gguf_dir = os.path.join(_hy_ckpts_dir(), "GGUF")
    ...
    def load_llm_gguf(self, gguf_file, device_strategy="gpu"):
        ...
        try:
            ...
            print(
                f"[HY-Motion] Initial memory status: {mem.available / (1024**3):.1f}GB available out of {mem.total / (1024**3):.1f}GB"
            )
        except:
            pass
        ...
        try:
            ...
            tokenizer = AutoTokenizer.from_pretrained(
                gguf_dir, padding_side="right", local_files_only=True
            )
                ...
                for f in os.listdir(qwen_dir):
                    ...
                    if os.path.isdir(full_path) and (
                        ...
                        try:
                            ...
                            tokenizer = AutoTokenizer.from_pretrained(
                                full_path, padding_side="right", local_files_only=True
                            )
                            tokenizer_loaded = True
                            print(f"[HY-Motion] Tokenizer loaded from: {full_path}")
                        except Exception as e:
                            print(
                                f"[HY-Motion] Failed to load tokenizer from {full_path}: {e}"
                            )

        # 尝试3: 从默认的Qwen3-8B目录加载tokenizer
        if not tokenizer_loaded:
            default_tokenizer_path = os.path.join(_hy_ckpts_dir(), "Qwen3-8B")
            if os.path.exists(default_tokenizer_path):
                try:
                    # 再次清理内存
                    gc.collect()
                    print(
                        f"[HY-Motion] Loading tokenizer from default path: {default_tokenizer_path}..."
                    )
                    tokenizer = AutoTokenizer.from_pretrained(
                    ...
                    tokenizer_loaded = True
                    print(
                        f"[HY-Motion] Tokenizer loaded from default path: {default_tokenizer_path}"
                    )
                except Exception as e:
                    print(
                        f"[HY-Motion] Failed to load tokenizer from default path: {e}"
                    )
        ...
        if has_native_gguf:
            ...
            try:
                ...
                mem = psutil.virtual_memory()
                print(
                    f"[HY-Motion] Memory status after cleanup: {mem.available / (1024**3):.1f}GB available out of {mem.total / (1024**3):.1f}GB"
                )
                ...
                if mem.available < 2 * 1024**3:  # 少于2GB可用内存
                    ...
                    mem = psutil.virtual_memory()
                    print(
                        f"[HY-Motion] Memory status after emergency cleanup: {mem.available / (1024**3):.1f}GB available"
                    )
            except:
                pass
            ...
            if device_strategy == "cpu":
                ...
                print(
                    "[HY-Motion] CPU mode: Loading model entirely on CPU to save GPU memory"
                )
                ...
                if device.type == "cuda":
                    ...
                    if torch.cuda.is_available():
                        ...
                        print(
                            f"[HY-Motion] Balanced mode: using up to {gpu_limit_gib}GiB of GPU memory and {cpu_limit_gib}GiB of CPU memory"
                        )
                        print(
                            "[HY-Motion] Using forced GPU device mapping for better memory management"
                        )
                else:
                    load_kwargs["device_map"] = "cpu"
                    load_kwargs["dtype"] = torch.float16  # 使用dtype替代torch_dtype
                ...
                if device.type == "cuda":
                    ...
                    if torch.cuda.is_available():
                        ...
                        print(
                            f"[HY-Motion] GPU mode: using up to {gpu_limit_gib}GiB of GPU memory and {cpu_limit_gib}GiB of CPU memory"
                        )
                        print(
                            "[HY-Motion] Forcing GPU device mapping to reduce CPU memory usage"
                        )
                else:
                    load_kwargs["device_map"] = "cpu"
                    load_kwargs["dtype"] = torch.float16  # 使用dtype替代torch_dtype
            ...
            try:
                # 加载前深度清理内存
                print(
                    "[HY-Motion] Performing deep memory cleanup before model loading..."
                )
                ...
                # 加载模型
                print(
                    f"[HY-Motion] Attempting to load model with kwargs: {load_kwargs.keys()}"
                )
                ...
                if "out of memory" in str(e).lower() or "memory" in str(e).lower():
                    ...
                    try:
                        ...
                        cpu_memory_limit = min(
                            int(total_system_memory * 0.4 / (1024**3)), 16
                        )  # 最多使用16GB CPU内存

                        print(
                            f"[HY-Motion] System memory: {total_system_memory / (1024**3):.1f}GB total"
                        )
                        print(
                            f"[HY-Motion] Setting CPU memory limit: {cpu_memory_limit}GiB"
                        )
                    except ImportError:
                        # psutil不可用，使用默认值
                        print(
                            "[HY-Motion] psutil not available, using default CPU memory limit"
                        )
                        cpu_memory_limit = 12  # 默认使用12GB CPU内存
                        print(
                            f"[HY-Motion] Setting default CPU memory limit: {cpu_memory_limit}GiB"
                        )
                    ...
                    for param in incompatible_params:
                        if param in cpu_load_kwargs:
                            ...
                            print(
                                f"[HY-Motion] Removed incompatible parameter from CPU fallback: {param}"
                            )

                    model = AutoModelForCausalLM.from_pretrained(
                        gguf_dir, **cpu_load_kwargs
                    )
                    model = model.eval().requires_grad_(False)
                ...
                if device_strategy == "gpu" and torch.cuda.is_available():
                    ...
                    try:
                        ...
                        cpu_memory_limit = min(
                            int(total_system_memory * 0.2 / (1024**3)), 8
                        )  # 最多使用8GB CPU内存
                        gpu_memory_limit = int(
                            torch.cuda.get_device_properties(device.index).total_memory
                            * 0.9
                            / (1024**3)
                        )  # 使用90% GPU内存

                        print(
                            f"[HY-Motion] System memory: {total_system_memory / (1024**3):.1f}GB total, {available_system_memory / (1024**3):.1f}GB available"
                        )
                        print(
                            f"[HY-Motion] Setting memory limits: GPU={gpu_memory_limit}GiB, CPU={cpu_memory_limit}GiB"
                        )
                    except ImportError:
                        # psutil不可用，使用默认值
                        print(
                            "[HY-Motion] psutil not available, using default memory limits"
                        )
                        ...
                        print(
                            f"[HY-Motion] Setting default memory limits: GPU={gpu_memory_limit}GiB, CPU={cpu_memory_limit}GiB"
                        )
                    ...
                    # 打印详细信息
                    print(
                        "[HY-Motion] Forcing GPU device mapping and reducing CPU memory usage"
                    )
                    print(
                        "[HY-Motion] This will attempt to perform more operations directly on GPU"
                    )
                    ...
                    for param in incompatible_params:
                        if param in optimized_load_kwargs:
                            ...
                            print(
                                f"[HY-Motion] Removed incompatible parameter from optimized GPU fallback: {param}"
                            )

                    try:
                        model = AutoModelForCausalLM.from_pretrained(
                            gguf_dir, **optimized_load_kwargs
                        )
                        model = model.eval().requires_grad_(False)
                        del optimized_load_kwargs
                        gc.collect()
                        print(
                            "[HY-Motion] Model loaded successfully in optimized GPU mode"
                        )
                        ...
                        try:
                            ...
                            # 系统内存充足（32G+），可以使用更多CPU内存
                            cpu_memory_limit = min(
                                int(total_system_memory * 0.3 / (1024**3)), 12
                            )  # 最多使用12GB CPU内存
                            ...
                            print(
                                f"[HY-Motion] Setting minimal CPU memory limit: {cpu_memory_limit}GiB"
                            )
                        except ImportError:
                            # psutil不可用，使用默认值
                            print(
                                "[HY-Motion] psutil not available, using default minimal CPU memory limit"
                            )
                            cpu_memory_limit = 8  # 默认使用8GB CPU内存
                            print(
                                f"[HY-Motion] Setting default minimal CPU memory limit: {cpu_memory_limit}GiB"
                            )
                        ...
                        for param in incompatible_params:
                            if param in minimal_load_kwargs:
                                ...
                                print(
                                    f"[HY-Motion] Removed incompatible parameter from minimal fallback: {param}"
                                )

                        model = AutoModelForCausalLM.from_pretrained(
                            gguf_dir, **minimal_load_kwargs
                        )
                        model = model.eval().requires_grad_(False)
                        ...
                        print(
                            "[HY-Motion] Model loaded successfully in minimal memory mode"
                        )
                else:
                    # 使用最小内存模式参数
                    # 获取系统内存信息
...
class HYMotionLoadNetwork:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "model_name": (
                    ["HY-Motion-1.0", "HY-Motion-1.0-Lite"],
    ...
    def load_network(self, model_name):
        ...
        if not _hymotion_complete(model_dir):
            if os.path.isdir(model_dir):
                # Incomplete/corrupt checkpoint (e.g. interrupted download) -
                # purge so the snapshot re-download actually happens.
                print(f"[HY-Motion] Removing incomplete model dir: {model_dir}")
                import shutil
        ...
        if os.path.exists(ckpt_path):
            ...
            network_state = {
                k.replace("motion_transformer.", ""): v
                for k, v in state_dict.items()
                if k.startswith("motion_transformer.")
            }
        ...
        wrapper = HYMotionNetworkWrapper(
            network=network, config=config, mean=mean, std=std, body_model=WoodenMesh()
        )
...
class HYMotionLoadPrompter:
    ...
    def INPUT_TYPES(s):
        ...
        prompter_dir = os.path.join(_hy_ckpts_dir(), "Text2MotionPrompter")
    ...
    def load_prompter(self, model_source, offload_to_cpu=False):
        ...
        if model_source == "local: Text2MotionPrompter":
            model_path = os.path.join(_hy_ckpts_dir(), "Text2MotionPrompter")
            if not os.path.exists(model_path):
                raise FileNotFoundError(
                    f"Prompter model directory not found: {model_path}. Please download the Text2MotionPrompter model first and place it in the HY-Motion models directory."
                )
        else:
        ...
        print(
            f"[HY-Motion] Loading Prompter: {model_path}, offload_to_cpu={offload_to_cpu}"
        )
...
class HYMotionRewritePrompt:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "text": (
                    "STRING",
                    {"default": "A person is walking forward.", "multiline": True},
                ),
            },
        }
    ...
    FUNCTION = "rewrite"
    CATEGORY = "OpenBlender/HY-Motion/Conditioning"
...
class HYMotionEncodeText:
    ...
    def _ensure_clip_loaded(self):
        ...
        if HYMotionEncodeText._clip_model is None:
            ...
            for variant in clip_variants:
                test_path = os.path.join(_hy_ckpts_dir(), variant)
                if os.path.isdir(test_path):
                    # Skip incomplete/corrupt dirs (e.g. interrupted download):
                    # a usable CLIP dir has config + tokenizer + weights.
                    has_config = os.path.isfile(os.path.join(test_path, "config.json"))
                    has_tokenizer = os.path.isfile(
                        os.path.join(test_path, "tokenizer_config.json")
                    )
                    has_weights = any(
                    ...
                    if has_config and has_tokenizer and has_weights:
                        ...
                        break
                    print(f"[HY-Motion] Skipping incomplete CLIP dir: {test_path}")
            ...
            HYMotionEncodeText._clip_model = CLIPTextModel.from_pretrained(
                local_path, local_files_only=True
            )
            HYMotionEncodeText._clip_model = (
                HYMotionEncodeText._clip_model.eval().requires_grad_(False)
            )
    ...
    def encode(self, llm: HYMotionLLMWrapper, text: str):
        ...
        enc = HYMotionEncodeText._clip_tokenizer(
            text_list, truncation=True, max_length=77, padding=True, return_tensors="pt"
        )
        ...
        with torch.no_grad():
            ...
            vtxt_raw = (
                ...
                else out.last_hidden_state.mean(1, keepdim=True)
            )
        ...
        with torch.no_grad():
            ...
            ctxt_raw = (
                llm_out.hidden_states[-1][
                    :, llm.crop_start : llm.crop_start + llm.max_length
                ]
                .contiguous()
                .to(device)
            )
            ctxt_length = (
                (llm_enc["attention_mask"].sum(dim=-1) - llm.crop_start)
                .clamp(0, llm.max_length)
                .to(device)
            )
        ...
        if ctxt_raw.shape[-1] != expected_dim:
            print(
                f"[HY-Motion] Converting ctxt_raw from {ctxt_raw.shape} to expected {expected_dim} dimensions"
            )

            # 创建一个线性转换层，将当前维度转换为4096，并确保dtype与输入一致
            conversion_layer = torch.nn.Linear(
                ctxt_raw.shape[-1], expected_dim, device=device, dtype=ctxt_raw.dtype
            )
...
class HYMotionGenerate:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "duration": (
                    "FLOAT",
                    {"default": 3.0, "min": 0.5, "max": 12.0, "step": 0.1},
                ),
    ...
    FUNCTION = "generate"
    CATEGORY = "OpenBlender/HY-Motion"

    def generate(
        self,
        network: HYMotionNetworkWrapper,
        ...
        length = min(
            max(int(duration * network.output_mesh_fps), 20), network.train_frames
        )
        ...
        if do_cfg:
            ...
            if network.null_vtxt_feat.shape[-1] != vtxt_input.shape[-1]:
                print(
                    f"[HY-Motion] Resizing null_vtxt_feat from {network.null_vtxt_feat.shape} to match {vtxt_input.shape}"
                )
                # 使用随机初始化的张量，维度匹配vtxt_input
                network.null_vtxt_feat = torch.randn(
                    1, 1, vtxt_input.shape[-1], device=device
                )

            # 确保null_ctxt_input的维度与ctxt_input匹配
            if network.null_ctxt_input.shape[-1] != ctxt_input.shape[-1]:
                print(
                    f"[HY-Motion] Resizing null_ctxt_input from {network.null_ctxt_input.shape} to match {ctxt_input.shape}"
                )
                # 使用随机初始化的张量，维度匹配ctxt_input
                network.null_ctxt_input = torch.randn(
                    1, 1, ctxt_input.shape[-1], device=device
                )

            # 现在可以安全地拼接
            vtxt_input = torch.cat(
                [network.null_vtxt_feat.expand_as(vtxt_input), vtxt_input], dim=0
            )
            ctxt_input = torch.cat(
                [network.null_ctxt_input.expand_as(ctxt_input), ctxt_input], dim=0
            )
        ...
        y0 = torch.cat(
            [
                randn_tensor(
                    (1, network.train_frames, network.input_dim),
    ...
    def _decode(self, latent, net):
        ...
        transl = x[..., :3].clone()
        rot6d = torch.cat(
            [x[..., 3:9].reshape(B, L, 1, 6), x[..., 9 : 9 + 126].reshape(B, L, 21, 6)],
            dim=2,
        )
        ...
        rot6d_s = rotation_matrix_to_rot6d(
            quaternion_to_matrix(
                torch.from_numpy(smooth_rotation(qq.cpu().numpy(), sigma=1.0))
            )
        ).to(latent.device)
        ...
        if net.body_model:
            ...
            with torch.no_grad():
                for b in range(B):
                    out = net.body_model.forward(
                        {"rot6d": rot6d_cpu[b], "trans": transl_cpu[b]}
                    )
        ...
        return {
            ...
            "rot6d": rot6d_s.cpu(),
            "transl": transl_s.cpu(),
            "root_rotations_mat": rot6d_to_rotation_matrix(
                rot6d_s[:, :, 0].cpu()
            ).cpu(),
        }
...
class HYMotionPreview:
    ...
    def render(self, motion_data, sample_index=0, frame_step=5, image_size=512):
        ...
        frames = [
            self._draw(kpts[i], image_size) for i in range(0, len(kpts), frame_step)
        ]
    ...
    def _draw(self, kpts, size):
        ...
        def px(p):
            return int((p[0] - cx) / scale * size + size / 2), int(
                size / 2 - (p[1] - cy) / scale * size
            )
...
class HYMotionSaveNPZ:
    ...
    def save(self, motion_data, output_dir, filename_prefix):
        ...
        for i in range(motion_data.batch_size):
            data = {
                k: motion_data.output_dict[k][i].cpu().numpy()
                if hasattr(motion_data.output_dict[k][i], "cpu")
                else motion_data.output_dict[k][i]
                for k in ["keypoints3d", "rot6d", "transl", "root_rotations_mat"]
                if k in motion_data.output_dict
            }
            data.update(
                {
                    "text": motion_data.text,
                    "duration": motion_data.duration,
                    "seed": motion_data.seeds[i] if i < len(motion_data.seeds) else 0,
                }
            )
...
class HYMotionExportFBX:
    ...
    def _resolve_fbx_path(self, custom_fbx_path):
        ...
        if path.startswith("input/") or path.startswith("output/"):
            ...
            print(
                f"[HY-Motion] DEBUG: path has input/output prefix, resolved = '{resolved}'"
            )
        else:
            # Default to input/ prefix
            resolved = os.path.join(comfy_root, "input", path)
            print(
                f"[HY-Motion] DEBUG: no prefix, defaulting to input, resolved = '{resolved}'"
            )
    ...
    def export(
        ...
        if resolved_fbx_path:
            ...
            print(
                f"[HY-Motion] Using RETARGET mode with custom FBX: {resolved_fbx_path}"
            )
            return self._export_with_retarget(
                motion_data,
                out_dir,
                filename_prefix,
                ts,
            ...
            return self._export_wooden_boy(
                motion_data, out_dir, filename_prefix, ts, uid
            )
    ...
    def _export_wooden_boy(self, motion_data, out_dir, filename_prefix, ts, uid):
        ...
        template_path = os.path.join(
            CURRENT_DIR, "assets", "wooden_models", "boy_Rigging_smplx_tex.fbx"
        )

        if (
            HYMotionExportFBX._fbx_converter is None
            or HYMotionExportFBX._fbx_converter_path != template_path
        ):
            try:
                import fbx
                from .hymotion.utils.smplh2woodfbx import SMPLH2WoodFBX

                HYMotionExportFBX._fbx_converter = SMPLH2WoodFBX(
                    template_fbx_path=template_path
                )
        ...
        for i in range(motion_data.batch_size):
            try:
                ...
                path = os.path.join(
                    out_dir, f"{filename_prefix}_{ts}_{uid}_{i:03d}.fbx"
                )
                success = HYMotionExportFBX._fbx_converter.convert_npz_to_fbx(
                    smpl_data, path
                )
        ...
        for i in range(motion_data.batch_size):
            try:
                ...
                for key in ["keypoints3d", "rot6d", "transl", "root_rotations_mat"]:
                    if (
                        key in motion_data.output_dict
                        and motion_data.output_dict[key] is not None
                    ):
...
class HYMotionPreviewAnimation:
    ...
    def preview(self, motion_data: HYMotionData, sample_index: int = 0):
        ...
        print(
            f"[HY-Motion] Preview Animation: {num_frames} frames, {num_joints} joints"
        )
